...
| **T-918** | **done** (after the close-out) | **Q-66 answered by the owner**: the reference and `openapi.json` need no key in Development, `health:read` everywhere else (FR-A-18's literal rule). `ScopeFor` gains `isDevelopment`, default **false**, so every other caller stays strict. Startup warns when the docs are served unauthenticated. Q-67 (Bearer) deliberately untouched |
| **T-917** | **done** (after the close-out) | `requireBackup` was a second decorative field - in the spec's own FR-A-5 example and read by nothing. Now raises `backupFreshness` to an error (stale backup, or no backup folder, => 422). Default unchanged. SPEC-GAP, **Q-72**. T-916's guard extended to the endpoint request records, which is what found it |
